...
class ProductBloc extends BlocBase{
  ...
  ProductBloc({this.categoryId,this.product}){
    if(product != null){
      ...
      unsavedData["images"] = List.of(product.get("images")); //cria lista de imagens expancivel
  ...
  Future<bool> saveProduct() async {
    ...
    try {
      if(product != null){ //produto está criado, atualizar produto
        ...
        DocumentReference dr = await FirebaseFirestore.instance.collection("products").doc(categoryId).
          collection("items").add(Map.from(unsavedData)..remove("images")); //cria o products no firebase sem o images
  ...
  Future _uploadImages(String productId) async {
    for(int i =0; i < unsavedData["images"].length; i++) {
      ...
      //upload da imagem na pasta categoria/produto/ com nome de data+hora
      UploadTask uploadTask = FirebaseStorage.instance.ref().child(categoryId).
          ...
          putFile(unsavedData["images"][i]); /*salva arquivo no firebase*/
      //TaskSnapshot s = await uploadTask.onComplete;
      TaskSnapshot s = await uploadTask.whenComplete(() => null);
      //recebe a url da imagem e escreve no formulario temporario
